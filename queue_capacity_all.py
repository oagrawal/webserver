...
def get_time_taken_for_tests(cargo_args, command):
    """Run the server with given args and benchmark it"""
    print(f"Starting server with args: {' '.join(cargo_args)}")
    # Start server with output suppressed
    server_process = subprocess.Popen(
        ["./target/debug/server"] + cargo_args,
    ...
    try:
        ...
        for i in range(num_tries):
            ...
            if match:
                ...
                return None
        return total / num_tries  # Return average time taken
...
def main():
    ...
    for command in cs:
        ...
        sequential_time = get_time_taken_for_tests(["1"], command)
        ...
        for capacity in internal_capacities:
            ...
            parallel_time = get_time_taken_for_tests(["2", "-q", str(capacity)], command)
